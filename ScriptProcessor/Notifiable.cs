...
namespace InfinityScript
{
    public abstract class Notifiable
    {
        ...
        public event Action<int, string, Parameter[]> Notified;
        ...
        internal void ProcessNotifications()
        {
            ...
            foreach (NotifyData notify in notifys)
            {
                    Notified?.Invoke(notify.entity, notify.type, notify.parameters);
                    WaitTillEvents?.Invoke(notify.type);
        ...
        {
            if (WaitTillEvents != null || Notified != null || _notifyHandlers.ContainsKey(type))
            {
